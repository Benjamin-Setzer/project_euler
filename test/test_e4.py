...
sys.path.append('../.')
import unittest
from e4 import is_pal, find_largest_pal_product

class Test_is_pal(unittest.TestCase):
	def test_true(self):
		for i in [9, 11, 121, 4004, 333, 805508]:
			self.assertTrue(is_pal(i), msg="funtion returned False, but %d is pal" %i)
	
	def test_false(self):
		for i in [12, 13, 42, 69, 5505]:
			self.assertFalse(is_pal(i), msg="funtion returned True, but %d is not pal" %i)
